...
Using `minikube`, bring up a Senzing stack on Kubernetes using Helm, RabbitMQ, and a SQLite database.
...
1. Install chart.
   Example:
    ...
    helm install \
      ...
      --namespace ${DEMO_NAMESPACE} \
      --values ${GIT_REPOSITORY_DIR}/helm-values/senzing-console-sqlite.yaml
    ```
...
1. Install chart.
   Example:
    ...
    helm install \
      ...
      --namespace ${DEMO_NAMESPACE} \
      --values ${HELM_VALUES_DIR}/senzing-api-server-sqlite.yaml
    ```
...
1. SQLite Web will be viewable at [localhost:9174](http://localhost:9174).
