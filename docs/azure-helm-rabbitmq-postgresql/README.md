...
using Microsoft Azure Kubernetes Service, `kubectl`, and `helm`.
...
1. [Prerequisites](#prerequisites)
    ...
    1. [View Kubernetes](#view-kubernetes)
...
1. Make a directory for the demo.
   Example:
    ...
    ```console
    export SENZING_DEMO_DIR=~/senzing-azure-rabbitmq-postgresql-demo-${DEMO_PREFIX}
...
1. :pencil2: When using a separate terminal window in each of the examples below, set environment variables.
   **Note:** Replace `${DEMO_PREFIX}` with the actual DEMO_PREFIX value.
   Example:
    ...
    ```console
    source ~/senzing-azure-rabbitmq-postgresql-demo-${DEMO_PREFIX}/environment.sh
    ```
